...
function Icon({ name, size = 18 }) {
  const paths = {
    ...
    copy: <><path d="M20 9H11a2 2 0 0 0-2 2v10a2 2 0 0 0 2 2h9a2 2 0 0 0 2-2v-10a2 2 0 0 0-2-2Z" /><path d="M4 15H3a2 2 0 0 1-2-2V3a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1" /></>,
...
function StatCard({ label, value, change, accent = 'green', icon }) {
  return <div className="stat-card">
    ...
    <div><span className="stat-label">{label}</span>
...
export default function App() {
  ...
  useEffect(() => { fetchConfig(); fetchOrders(); mockPriceData() }, [])

  function mockPriceData() {
    const prices = { 'SOL/USDT': 182.45, 'BTC/USDT': 42835.00, 'ETH/USDT': 2284.50, 'DOGE/USDT': 0.38, 'TRX/USDT': 0.21, 'ARB/USDT': 1.02 }
    setCurrentPrice(prices['SOL/USDT'])
  }
  ...
  async function handleSave(event) {
    ...
    setLoading(false)
    setNotice(error ? `Error al guardar: ${error.message}` : '✓ Configuración guardada')
  ...
  return <div className="app-shell">
    ...
    <main className="main-content">
      ...
      <div className="status-banner">
        ...
        <button className={`banner-button ${config.is_active ? 'pause' : 'play'}`} onClick={() => setField('is_active', !config.is_active)}>{config.is_active ? '⏸ Pausar' : '▶ Activar'}</button>
      </div>
          ...
          <div className="chart-area">
            ...
            <div className="line-chart">
              ...
              <svg viewBox="0 0 700 200" preserveAspectRatio="none">
                <defs><linearGradient id="gradientFill" x1="0" x2="0" y1="0" y2="1"><stop offset="0" stopColor="#16d4a0" stopOpacity=".25" /><stop offset="1" stopColor="#16d4a0" stopOpacity="0" /></linearGradient></defs>
        ...
        <section className="panel config-panel" id="config-form">
          ...
          <form onSubmit={handleSave} className="form-grid">
            <label className="form-group">
              ...
              <select value={config.symbol} onChange={e => { setField('symbol', e.target.value); mockPriceData() }}>
            ...
            <label className="toggle-label">
              <input type="checkbox" checked={config.is_active} onChange={e => setField('is_active', e.target.checked)} />
